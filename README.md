...
- Verify with `GET /api/health`, NOT by grepping the page title. A misconfigured daemon serves a perfectly good dashboard while every task dies — "the page loads" is not evidence that anything works.
  ...
  If `ok` is `false`, the `notices` array in that response says exactly what is wrong and what to do about it — read it back to me and STOP. Do NOT report a successful install. If port 8787 is already in use, the service exits 78 with a one-line reason; tell me (I can set a different AGENTDECK_PORT).
